...
describe('stravaApi', () => {
  it('odczytuje status i aktywności z prywatnych endpointów', async () => {
    ...
    expect(await stravaActivities(fetchImpl)).toMatchObject({ activities: [{ id: '1' }], status: 200 });
    ...
    expect(fetchImpl).toHaveBeenNthCalledWith(2, '/api/strava/activities', expect.objectContaining({ credentials: 'same-origin', cache: 'no-store' }));
  });

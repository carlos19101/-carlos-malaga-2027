        ...
        <div className="dashboard-disclosure-stack">
          ...
          <DashboardDisclosure eyebrow="LOAD MAP" title="Wielowymiarowa mapa obciążenia" summary={loadMap.state === 'missing' ? 'brak danych z Training Log' : `${formatMetricNumber(loadMap.running.km7, { maximumFractionDigits: 2 })} km · ${formatRunCount(loadMap.running.count7)} / 7 dni`}>
            ...
            <div className="load-grid load-map-grid">
              ...
              <StatCard label="BOKS / SIŁA / MOB. · 7D" value={loadMap.systemic.state === 'missing' ? '' : `${loadMap.systemic.boxing7} / ${loadMap.systemic.strength7} / ${loadMap.systemic.mobility7}`} note="liczone osobno; bez sztucznego przeliczenia na kilometry" />
  ...
  return (
    <section className="section-block strava-section">
      ...
      <div className="feedback-card strava-card">
        ...
        {state.message ? <p className="feedback-message" role="status">{state.message}</p> : null}
        {state.activities.length ? <><p className="strava-reconciliation">Bieżący okres od {coverageStartDate || 'braku daty'}: <b>{reconciliation.summary.matched} zgodne</b> · <b>{currentReviewCount} do weryfikacji</b>. Czas ruchu Stravy nie zastępuje czasu całej aktywności z TCX.</p>{currentEntries.length ? <div className="strava-activity-list">{currentEntries.map(renderEntry)}</div> : <p className="strava-current-empty">Brak aktywności Stravy w bieżącym okresie do pokazania.</p>}{historicalEntries.length ? <details className="strava-history"><summary>Historia Stravy · {historicalEntries.length} wpisów sprzed Training Log</summary><p>Wpisy historyczne pozostają dostępne jako kontekst, ale nie są używane do bieżącej kontroli realizacji planu.</p><div className="strava-activity-list">{historicalEntries.map(renderEntry)}</div></details> : null}</> : null}
      </div>
    </section>
  );
}
...
function App() {
  ...
  if (!feedbackAccess.checked || feedbackAccess.configured === null || !feedbackAccess.configured
    ...
    return <AccessGate access={feedbackAccess} onLogin={loginFeedback} onRetry={retryAccess} appVersion={APP_VERSION} />;
  }
